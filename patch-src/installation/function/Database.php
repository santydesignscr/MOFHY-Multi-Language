...
$sql = mysqli_query($connect,'CREATE TABLE IF NOT EXISTS `hosting_area` (
  ...
  `area_logo` INT(1) NOT NULL,
  `area_editor` INT(1) NOT NULL
)');
...
$sql = mysqli_query($connect,'CREATE TABLE IF NOT EXISTS `hosting_clients` (
  ...
  `hosting_client_state` VARCHAR(50) NOT NULL,
  `hosting_client_password` VARCHAR(100) NOT NULL,
  `hosting_client_type` VARCHAR(100) NOT NULL
)');
...
$sql = mysqli_query($connect,'CREATE TABLE IF NOT EXISTS `hosting_ssl` (
  ...
  `ssl_privater_key` VARCHAR NOT NULL
)');
...
$sql = mysqli_query($connect,"CREATE TABLE IF NOT EXISTS `hosting_visits` (
 `visit_id` INT(11) PRIMARY KEY AUTO_INCREMENT NOT NULL ,
 `visit_device` VARCHAR(50) NOT NULL,
...
if(!mysqli_num_rows($sql)>0){
  $sql = mysqli_query($connect,"INSERT INTO `hosting_smtp`(`smtp_key`, `smtp_host`, `smtp_username`, `smtp_password`, `smtp_port`, `smtp_from`, `smtp_name`) VALUES ('SMTP','smtp.gmail.com','example@gmail.com','example123','587','example@gmail.com','Example')");
}
...
$sql = mysqli_query($connect,"SELECT * FROM `hosting_domain_extensions` WHERE `extension_value`='.example.com'");
if(!mysqli_num_rows($sql)>0){
