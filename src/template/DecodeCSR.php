			...
			<div class="row pb-20">
				...
				<div class="col-md-12">
					<div class="mb-10 px-10">
						...
						<textarea name="csr" style="height: 250px" placeholder="<?php echo $text['csr_code_place'];?>" class="form-control" id="csr_code" required></textarea> 
					</div>
				</div>
				<div class="col-md-12">
					<div class="mb-5 px-10">
						<button name="submit" id="decode" class="btn btn-sm btn-primary"><?php echo $text['decode_csr'];?></button>
					</div>
				</div>
...
$(document).ready(function(){
	...
	var csr_code = $("#csr_code").val();
		$.post("function/DecodeCSR.php", {csr: csr_code,submit: ""}, function(data){
			$('#hidden-area').html('<div class="alert alert-success" role="alert"><button class="close" data-dismiss="alert" type="button" aria-label="Close"><span aria-hidden="true">&times;</span></button><?php echo $text['decode_csr_success'];?></div>');
			$("#result").html(data);
		});
	});
});
